...
# Projeto Final
# EDP Hiperbólica - Forward in Time, Backward in Space (FTBS)
...
# Figura 1 - Ondas Notáveis
def fig1(result):
    fig = plt.figure()
    fig.suptitle(f"Forward in Time, Backward in Space (FTBS) - Ondas Notáveis")
    for i in range(9):
...
def fig2(result):
    fig= plt.figure()
    fig.suptitle(f"Forward in Time, Backward in Space (FTBS) - Sobrepostas")
...
def fig3(result):
    fig = plt.figure()
    fig.suptitle(f"Forward in Time, Backward in Space (FTBS) - Ondas 3D")
    for i in range(len(result)):
        ...
        ax.plot_surface(Y,X,W,cmap="jet")
...
def ani(result):
    ...
    # anim.save(f"ani1.gif", writer=writer)

######################################################

# Tabela - Resultados Notáveis
def tab1(result):
    print(f"Forward in Time, Backward in Space (FTBS)")
    tabela = PrettyTable()
...
def ftbs(x_0,x_L,t_0,t_T,m,n,a,v):
    ...
    # Passo 2
    x = np.linspace(x_0,x_L,m+1)    # Discretização do espaço
    
    
    # Passo 3
    sol = []                                # Vetor que armazena s soluções
    u0 = v(x)                               # Onda inicial
    u = u0.copy()                           # Variável temporária
    for _ in range(n+1):                    # Loop temporal
        u = u -Lambda*(u - np.roll(u,1))    # MDF - FTBS
        sol.append(u)                       # Armazena a solução atual

    # Solução exata
    ue = v(x+k*n)

    # Erro
    erro = max(abs(u-ue))   # Norma Infinito

    # Tempo
    tempo = time.time() - ti    # Tempo de processamento
...
def Q1p1():
    ...
    for i in range(len(m)):
        result.append(ftbs(x_0,x_L,t_0,t_T,m[i],n[i],a,v))
